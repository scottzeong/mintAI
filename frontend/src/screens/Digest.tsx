 ...
 * 이 좌우 비대칭이 화면에서 눈으로 보여야 한다. 그래서 좌측은 흐리게, 우측은
 * 흰 배경에 또렷하게 둔다.
 *
 * 검증 대상은 H1 — **"AI 자료를 읽고 직접 요약하는 마찰을 감수할 만한가."**
 * 붙여넣기를 막는 게 불편해서 이 화면을 안 쓰게 된다면, 그 자체가 H1의 답이다.
 */
export default function Digest() {
  ...
  const refreshQueue = useCallback(async () => {
    const [awaiting, inbox] = await Promise.all([
      listIdeas('awaiting_digest', 50),
      listIdeas('inbox', 50),
    ])
    setQueue([...awaiting, ...inbox])
  }, [])

  useEffect(() => {
    void refreshQueue().catch(() => setError('큐를 불러오지 못했습니다'))
    return () => {
  ...
  async function research(idea: Idea) {
    ...
    setError(null)
    try {
      await startResearch(idea.id)
      // §4.1 — 진행 상황은 ideas.status 하나로 표현한다. 2초 폴링으로 충분하다.
      const poll = async () => {
        const fresh = (await listIdeas(undefined, 200)).find((i) => i.id === idea.id)
        if (fresh?.status === 'awaiting_digest') {
          ...
          setBusy(false)
          const r = await getRun(idea.id).catch(() => null)
   ...
   * 차단만 하고 끝내지 않고 `paste_blocked` 를 남기는 이유는 §2.2 참조 —
